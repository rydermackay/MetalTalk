...
//  Copyright © 2015 Flixel Photos Inc. All rights reserved.
//
...
{
    float4 position [[position]]; // x, y, z, 1/w in window coordinates when handed to fragment function
    float3 eye;
    float3 normal;
    float2 texCoords;
};
...
{
    float4 baseColor = texture.sample(s, vert.texCoords);
    float3 ambientTerm = light.ambientColor * baseColor.rgb;
    
    float3 normal = normalize(vert.normal);
    float diffuseIntensity = saturate(dot(normal, light.direction));
    float3 diffuseTerm = light.diffuseColor * baseColor.rgb * diffuseIntensity;
